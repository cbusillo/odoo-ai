...
class UnitTestCase(_ShopifyMockMixin, TransactionCase):
    ...
    def mock_shopify_client(self) -> Iterator[MagicMock]:
        with patch("odoo.addons.shopify_sync.services.shopify.service.ShopifyClient") as mock_client:
            instance = MagicMock()
...
class TourTestCase(MultiWorkerHttpCase):
    ...
    def setUpClass(cls) -> None:
        ...
        if db_name not in cls.assets_reset_db_names:
            cls.env["ir.attachment"].sudo().search([("url", "ilike", "/web/assets/")]).unlink()
            cls.assets_reset_db_names.add(db_name)
    ...
    def _setup_test_user(cls) -> None:
        ...
        if not test_user:
            ...
            mail_user_group_xml_id = ".".join(["mail", "group_mail_user"])
            try:
                mail_user_group = cls.env.ref(mail_user_group_xml_id)
            except ValueError:  # External ID not found when mail isn't installed
                mail_user_group = None
            ...
            group_ids = [system_group.id]
            if mail_user_group:
                group_ids.append(mail_user_group.id)
            if stock_manager:
                group_ids.append(stock_manager.id)
            ...
            if mail_user_group and mail_user_group not in test_user.group_ids:
                to_add.append(mail_user_group.id)
