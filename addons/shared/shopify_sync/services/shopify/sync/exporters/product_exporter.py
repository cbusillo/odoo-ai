...
from ...gql.enums import FileContentType, ProductStatus, WeightUnit
...
from ...metafield_types import ShopifyMetafieldType
from ...helpers import (
...
class ProductExporter(ShopifyBaseExporter["odoo.model.product_product"]):
    ...
    def _export_one(self, odoo_product: "odoo.model.product_product") -> None:
        ...
        shopify_product_reference = odoo_product.external.shopify

        shopify_product_id = shopify_product_reference.product.id
        ...
        all_images_have_media_id = all(image.external.shopify.media.id for image in image_records) if image_records else False
    ...
    def _update_odoo_product(odoo_product: "odoo.model.product_product", shopify_product: ProductSetProductSetProduct) -> None:
        ...
        shopify_external = odoo_product.sudo().external.shopify
        shopify_external.product.id = shopify_product_id
        shopify_external.variant.id = shopify_variant_id
        shopify_external.condition.id = condition_id
        shopify_external.ebay_category.id = ebay_category_id
    ...
    def _sync_images_after_export(
        ...
        shopify_product: ProductSetProductSetProduct,
    ) -> None:
        shopify_media_nodes = shopify_product.media.nodes
        ordered_odoo_images = odoo_product.images.sorted(key=image_order_key)
        local_media_identifiers = {image.external.shopify.media.id for image in ordered_odoo_images if image.external.shopify.media.id}
        ...
        for odoo_image, shopify_image in zip(ordered_odoo_images, shopify_images):
            ...
            if not odoo_image.external.shopify.media.id:
                odoo_image.sudo().external.shopify.media.id = media_id
    ...
    def _verify_shopify_media_for_reorder(
        ...
        if not page.nodes:
            _logger.warning(
                "Shopify product %s missing during reorder verification. Scheduling media reupload and preserving product mappings.",
                shopify_product_id,
            )
    ...
    def _schedule_media_reupload(
        ...
        *,
        ...
        media_identifiers = {image.external.shopify.media.id for image in ordered_odoo_images if image.external.shopify.media.id}
    ...
    def _clear_shopify_media_ids(
        ...
        for image in ordered_odoo_images:
            media_identifier = image.external.shopify.media.id
            if media_identifier and media_identifier in media_identifiers_to_clear:
                image.sudo().external.shopify.media.id = None
    ...
    def _publish_product(self, shopify_product_gid: str) -> None:
        ...
        if not str2bool(self.env["ir.config_parameter"].sudo().get_param("shopify.test_store"), default=False):
    ...
    def _map_odoo_product_to_shopify_product_set_input(
        ...
        *,
        ...
        shopify_variant_set_input = ProductVariantSetInput(
            id=(
                format_shopify_gid_from_id("ProductVariant", shopify_product_reference.variant.id)
                if shopify_product_reference.variant.id
                else None
            ),
        ...
        should_upload_media = bool(image_records) and (
            not shopify_product_reference.product.id or images_need_update or force_media_upload
        )
        ...
        if not shopify_product_reference.product.id or odoo_product.shopify_next_export_quantity_change_amount:
        ...
        if odoo_product.condition.code:
            shopify_product_set_input.metafields += [
                self.metafield_from_id_value_key(
                    shopify_product_reference.condition.id,
                    "condition",
                    odoo_product.condition.code,
                    ShopifyMetafieldType.SINGLE_LINE_TEXT_FIELD,
                )
            ]

        if odoo_product.part_type and odoo_product.part_type.external.ebay.category.id:
            shopify_product_set_input.metafields += [
                self.metafield_from_id_value_key(
                    shopify_product_reference.ebay_category.id,
                    "ebay_category_id",
                    odoo_product.part_type.external.ebay.category.id,
                    ShopifyMetafieldType.NUMBER_INTEGER,
                )
            ]
    ...
    def _reorder_shopify_media(
        ...
        for new_position, odoo_image in enumerate(ordered_odoo_images):
            media_id = odoo_image.external.shopify.media.id
            if media_id:
                moves.append(
                    MoveInput(
