...
- Research existing implementations; prefer learning from core modules first.
- Confirm model info, inheritance, and view usage before designing changes.
- Keep changes minimal and verify with targeted tests and inspections.

Suggested steps

- Search code across modules for relevant patterns.
- Inspect model info (fields, inherits, depends) and view usage for the target model.
- Consider performance implications (batching, read_group, prefetch).
