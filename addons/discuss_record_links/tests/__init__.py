...
# Expose JavaScript unit tests in subpackage so Odoo test loader finds them.
try:
    from .js import js_unit_tests as javascript_unit_tests
except ImportError:
    javascript_unit_tests = None  # pragma: no cover - best-effort import

if javascript_unit_tests is not None:
    # Re-expose under a test_* name for Odoo discovery.
    sys.modules[__name__].test_js_units = javascript_unit_tests
