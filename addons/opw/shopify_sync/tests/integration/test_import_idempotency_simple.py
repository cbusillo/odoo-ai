...
class TestImportIdempotencySimple(IntegrationTestCase):
    ...
    def test_import_since_last_import_filters_correctly(self) -> None:
        ...
        old_time = common.datetime.now(timezone.utc) - common.timedelta(hours=2)
        new_time = common.datetime.now(timezone.utc)
        ...
        middle_time = old_time + common.timedelta(hours=1)
        ...
        def mock_fetch_page(_client: object, query: str | None, cursor: str | None) -> common.MagicMock:
        ...
        with common.patch.object(importer, "_fetch_page", side_effect=mock_fetch_page):
            count = importer.import_orders_since_last_import()
            self.assertEqual(count, 1)

        order_external_ids = self.env["external.id"].search(
            [
                ("res_model", "=", "sale.order"),
                ("system_id.code", "=", "shopify"),
                ("resource", "=", "order"),
                ("active", "=", True),
            ]
        )
        orders = self.env["sale.order"].browse(order_external_ids.mapped("res_id"))
        self.assertEqual(len(orders), 1)
        self.assertEqual(orders[0].external_id, "1102")
    ...
    def test_first_import_gets_all_data(self) -> None:
        ...
        def mock_fetch_page(_client: object, _query: str | None, cursor: str | None) -> common.MagicMock:
            mock_page = common.MagicMock()
        ...
        orders = self.env["sale.order"].search_by_bound_external_id("1103")
        self.assertEqual(len(orders), 1)
