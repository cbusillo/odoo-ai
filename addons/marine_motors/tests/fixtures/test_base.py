...
class ProductConnectTransactionCase(TransactionCase):
    ...
    def _create_multigraph_test_products(cls) -> None:
        ...
        product_model = cls.env["product.template"]
        products_to_create: list[dict[str, object]] = []
        for sequence in range(1, 5):
            values = {
                "name": f"Test Product {sequence}",
                "default_code": f"{10000 + sequence}",
                "list_price": 100 * sequence,
                "standard_price": 60 * sequence,
                "type": "consu",
                "is_ready_for_sale": True,
                "initial_quantity": 10 * sequence,
            }
            optional_fields = {
                "is_ready_for_sale_last_enabled_date": date(2025, 1, sequence),
                "initial_price_total": 1000 * sequence,
                "initial_cost_total": 600 * sequence,
            }
        ...
        cls.test_products = product_model.create(products_to_create)
