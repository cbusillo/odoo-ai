...
def _migrate_template_shopify_product_ids(env: api.Environment) -> dict[str, set[str]]:
    ...
    template_rows = _fetch_template_shopify_product_id_rows(
        env.cr,
        template_has_shopify_id=template_has_shopify_id,
        product_has_shopify_id=product_has_shopify_id,
    )
    ...
    for template_row in template_rows:
        variant_id = template_row["variant_id"]
        if variant_id is None:
            continue
        normalized_external_ids = template_row["normalized_external_ids"]
        if not normalized_external_ids:
            continue
        if len(normalized_external_ids) > 1:
            _logger.warning(
                "Preserving legacy Shopify product ID columns for product.template %s: conflicting IDs %s",
                template_row["template_id"],
                sorted(normalized_external_ids),
            )
            can_drop_legacy_columns = False
            continue

        normalized_external_id = next(iter(normalized_external_ids))
        variant_record = product_model.browse(variant_id).exists()
        if not variant_record:
            continue
        if not variant_record.set_external_id("shopify", normalized_external_id, "product"):
            _logger.warning(
                "Skipping duplicate Shopify product id '%s' for product.product %s: already used by %s",
                normalized_external_id,
                variant_id,
                product_model.search_by_external_id("shopify", normalized_external_id, resource="product").id,
            )
            can_drop_legacy_columns = False
...
def _migrate_external_ids_for_system(
    ...
    *,
    ...
    normalize_shopify_external_ids: bool = True,
) -> set[str]:
    ...
    for row in rows:
        ...
        for column_name, raw_value in zip(filtered_map.keys(), row[1:]):
            ...
            if system_code == "shopify" and normalize_shopify_external_ids:
            ...
            if not record.set_external_id(system_code, normalized_external_id, resource):
                _logger.warning(
                    ...
                    model_name,
                    resource,
                    record_model.search_by_external_id(system_code, normalized_external_id, resource).id,
                )
                drop_candidates.discard(column_name)
...
def _fetch_template_shopify_product_id_rows(
    cr: Cursor,
    *,
    template_has_shopify_id: bool,
    product_has_shopify_id: bool,
) -> list[dict[str, int | set[str] | None]]:
    if not template_has_shopify_id and not product_has_shopify_id:
        return []

    select_columns = ["pt.id AS template_id", "pp.id AS variant_id"]
    where_clauses: list[str] = []
    if template_has_shopify_id:
        select_columns.append("pt.shopify_product_id AS template_shopify_product_id")
        where_clauses.append("(pt.shopify_product_id IS NOT NULL AND pt.shopify_product_id != '')")
    else:
        select_columns.append("NULL AS template_shopify_product_id")
    if product_has_shopify_id:
        select_columns.append("pp.shopify_product_id AS variant_shopify_product_id")
        where_clauses.append("(pp.shopify_product_id IS NOT NULL AND pp.shopify_product_id != '')")
    else:
        select_columns.append("NULL AS variant_shopify_product_id")
    ...
    rows_by_template: dict[int, dict[str, int | set[str] | None]] = {}
    for template_id, variant_id, template_external_id, variant_external_id in cr.fetchall():
        template_row = rows_by_template.setdefault(
            template_id,
            {
                "template_id": template_id,
                "variant_id": variant_id,
                "normalized_external_ids": set(),
            },
        )
        current_variant_id = template_row["variant_id"]
        if current_variant_id is None or variant_id < current_variant_id:
            template_row["variant_id"] = variant_id
        for raw_value in (template_external_id, variant_external_id):
            sanitized = (raw_value if isinstance(raw_value, str) else str(raw_value or "")).strip()
            if not sanitized:
                continue
            normalized_external_id = _normalize_shopify_external_id(sanitized)
            if not normalized_external_id:
                continue
            template_row["normalized_external_ids"].add(normalized_external_id)

    return list(rows_by_template.values())
