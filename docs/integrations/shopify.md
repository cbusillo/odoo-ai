...
Callers should use the fluent `external_ids` API directly, for example
`record.external.shopify.product.id` or bound helpers like
`record.external_reference.id`, instead of addon-local alias properties or
legacy `shopify_*_id` model fields. See
