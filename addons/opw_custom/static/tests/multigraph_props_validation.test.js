...
import { multigraphView } from "@opw_custom/views/multigraph/multigraph_view"
import { MultigraphModel } from "@opw_custom/views/multigraph/multigraph_model"
import { MultigraphRenderer } from "@opw_custom/views/multigraph/multigraph_renderer"

test("@opw_custom multigraph view props function returns valid props structure", () => {
...
test("@opw_custom multigraph view props handles undefined gracefully", () => {
    const mockGenericProps = {
        arch: new DOMParser().parseFromString('<graph/>', 'text/xml').documentElement,
        fields: {}
    }
