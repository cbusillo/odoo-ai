...
class TestShopifyHelpers(UnitTestCase):
    ...
    def test_external_reference_get_ignores_archived_mapping(self) -> None:
        archived_partner = PartnerFactory.create(
            self.env,
            name="Archived Shopify Customer",
            shopify_customer_id="9001",
        )
        archived_partner.get_external_id_record("shopify", "customer").active = False

        found_partner = self.env["res.partner"].external["shopify"]["customer"].get("9001")

        self.assertFalse(found_partner)

    def test_find_partner_by_shopify_address_id_ignores_archived_mapping(self) -> None:
        archived_address = PartnerFactory.create(
            self.env,
            name="Archived Shopify Address",
            shopify_address_id="8001",
        )
        archived_address.get_external_id_record("shopify", "address_invoice").active = False

        found_partner = helpers.find_partner_by_shopify_address_id(
            self.env,
            "8001",
            role="billing",
        )
