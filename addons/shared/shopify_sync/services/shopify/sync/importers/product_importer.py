...
class ProductImporter(ShopifyBaseImporter[ProductFields]):
    ...
    def _find_matching_odoo_product(
        ...
        product_match = product_model.external["shopify"]["product"].get(shopify_product_id)
        if product_match:
            return product_model.browse(product_match.id)

        variant = shopify_product.variants.nodes[0]
        shopify_variant_id = parse_shopify_id_from_gid(variant.id)
        variant_match = product_model.external["shopify"]["variant"].get(shopify_variant_id)
        if variant_match:
    ...
    def _link_missing_external_ids(
        ...
        for resource, external_id_value in desired_external_ids.items():
            if not external_id_value:
                continue
            if (shopify_product_reference[resource].id or None) == external_id_value:
                continue
            odoo_product.sudo().external["shopify"][resource].id = external_id_value
    ...
    def _images_are_in_sync(
        ...
        if len(template_images) != len(shopify_images):
            ...
            return False
        if any(not image.external.shopify.media.id for image in template_images):
    ...
    def get_or_create_part_type(self, part_type_name: str, ebay_category_id: str) -> "odoo.model.product_type":
        ...
        part_type_model: "odoo.model.product_type" = self.env["product.type"]
        part_type = part_type_model.external.ebay.category.get(ebay_category_id)
        if part_type:
            if part_type.name != part_type_name:
                part_type.name = part_type_name
            return part_type

        part_type = part_type_model.create({"name": part_type_name})
        part_type.external.ebay.category.id = ebay_category_id
    ...
    def _get_part_type(self, part_type_name: str, ebay_category_id: str) -> "odoo.model.product_type":
        ...
        part_type = self.env["product.type"].external.ebay.category.get(ebay_category_id)
        if part_type and part_type.name == part_type_name:
            return part_type
        return self.env["product.type"].browse()
    ...
    def _part_type_change_detected(self, odoo_product: "odoo.model.product_product", shopify_product: ProductFields) -> bool:
        ...
        if not matched_part_type:
            if not current_part_type:
                return True
            if current_part_type.external.ebay.category.id != ebay_category_id:
                return True
    ...
    def import_images_from_shopify(self, odoo_product: "odoo.model.product_product", shopify_product: ProductFields) -> None:
        ...
        for image in template_images:
            media_id = image.external.shopify.media.id
            if media_id:
        ...
        for shopify_image in shopify_images:
            ...
            image.sudo().external.shopify.media.id = media_id
    ...
    def _ordered_odoo_media_ids(product: "odoo.model.product_product") -> list[str]:
        ...
        for image in ordered_images:
            external_id = image.external.shopify.media.id
            if external_id:
    ...
    def save_odoo_product(
        ...
        *,
        ...
        try:
            ...
            odoo_product.sudo().external.shopify.product.id = shopify_product_id
            odoo_product.sudo().external.shopify.variant.id = shopify_variant_id
