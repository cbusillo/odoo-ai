...
class ShopifySync(models.TransientModel):
    ...
    @api.model
    def _is_shopify_test_store_enabled(self) -> bool:
        return str2bool(self.env["ir.config_parameter"].sudo().get_param("shopify.test_store"), default=False)
    ...
    def fields_get(self, allfields: list[str] | None = None, attributes: list[str] | None = None) -> dict[str, dict]:
        ...
        if (mode := resource.get("mode")) and (selection := mode.get("selection")) and not self._is_shopify_test_store_enabled():
    ...
    def _run_reset_shopify(self) -> None:
        ...
        if not self._is_shopify_test_store_enabled():
            raise UserError("Reset Shopify is only allowed when shopify.test_store is enabled.")
        ...
        products.write(
            {
                ...
                "shopify_last_exported_at": False,
                "shopify_next_export_quantity_change_amount": 0,
                "shopify_created_at": False,
            }
        )
        system = self.env["external.system"].search([("code", "=", "shopify")], limit=1)
        if system:
            external_ids = self.env["external.id"].sudo().with_context(active_test=False).search([("system_id", "=", system.id)])
            if external_ids:
