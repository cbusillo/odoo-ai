...
Shared PyCharm run configurations live in the tracked `.idea/runConfigurations/` directory. File names are auto
generated by PyCharm; use the run configuration names below.

- `OPW Local - Init` bootstraps the OPW stack with
  `uv run restore-from-upstream --stack opw-local --bootstrap-only`.
- `CM Local - Init` performs the same bootstrap for the CM stack so you
  start from a clean database and filestore.
- `OPW Local - Up` / `CM Local - Up` bring the corresponding stacks online after configuration changes.
- `OPW Local - Restore` / `CM Local - Restore` refresh local data from upstream.
- `OPW Testing - Restore` runs the upstream restore for the
...
1. The shared `CM Local - Up` run configuration brings the Docker stack online. After the
