...
def find_partner_by_shopify_address_id(
    ...
    *,
    ...
    for resource in address_resources:
        partner_record = partner_model.external["shopify"][resource].get(shopify_address_id)
...
class ShopifyApiError(OdooDataError):
    ...
    def __str__(self) -> str:
        ...
        if name := self.name:
            if name not in text:
                text += f" [{name}]"
        if shopify_product_id := self.shopify_product_id:
            text += f" [Shopify ID {shopify_product_id}]"
        if sku := self.sku:
            if f"[Shopify SKU {sku}]" not in text:
                text += f" [Shopify SKU {sku}]"
        return text
    ...
    def shopify_product_id(self) -> str:
        if self.shopify_record:
            return self._coerce_text(getattr(self.shopify_record, "id", ""))
        return ""

    @property
    def name(self) -> str:
        if self.shopify_record:
            return self._coerce_text(getattr(self.shopify_record, "title", ""))
        return self._coerce_text(super().name)
