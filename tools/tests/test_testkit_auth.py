...
class TestkitAuthTests(unittest.TestCase):
    def test_setup_test_authentication_hashes_and_updates_admin_password(self) -> None:
        table_probe_result = subprocess.CompletedProcess(args=[], returncode=0, stdout="t\n", stderr="")
        ...
        update_call = compose_exec_mock.call_args_list[2]
    ...
    def test_setup_test_authentication_skips_password_update_for_blank_database(self) -> None:
        hash_result = subprocess.CompletedProcess(args=[], returncode=0, stdout="pbkdf2$hash\n", stderr="")
        table_probe_result = subprocess.CompletedProcess(args=[], returncode=0, stdout="f\n", stderr="")
        with (
            patch("tools.testkit.auth.get_script_runner_service", return_value="script-runner"),
            patch("tools.testkit.auth.ensure_services_up"),
            patch("tools.testkit.auth.get_database_service", return_value="database"),
            patch("tools.testkit.auth.get_db_user", return_value="odoo"),
            patch("tools.testkit.auth.compose_exec", side_effect=[hash_result, table_probe_result]) as compose_exec_mock,
            patch.dict(os.environ, {}, clear=True),
        ):
            generated_password = setup_test_authentication("odoo-test")
            self.assertEqual(os.environ.get("ODOO_TEST_PASSWORD"), generated_password)

        self.assertEqual(compose_exec_mock.call_count, 2)
    ...
    def test_setup_test_authentication_fails_when_password_update_fails(self) -> None:
        ...
        with (
            ...
            patch("tools.testkit.auth.get_database_service", return_value="database"),
            patch("tools.testkit.auth.get_db_user", return_value="odoo"),
            patch("tools.testkit.auth.compose_exec", side_effect=[hash_result, table_probe_result, update_result]),
            patch.dict(os.environ, {}, clear=True),
        ):
    ...
    def test_setup_test_authentication_fails_when_table_probe_fails(self) -> None:
        hash_result = subprocess.CompletedProcess(args=[], returncode=0, stdout="pbkdf2$hash\n", stderr="")
        table_probe_result = subprocess.CompletedProcess(args=[], returncode=1, stdout="", stderr="connection lost")
        with (
            patch("tools.testkit.auth.get_script_runner_service", return_value="script-runner"),
            patch("tools.testkit.auth.ensure_services_up"),
            patch("tools.testkit.auth.get_database_service", return_value="database"),
            patch("tools.testkit.auth.get_db_user", return_value="odoo"),
            patch("tools.testkit.auth.compose_exec", side_effect=[hash_result, table_probe_result]),
            patch.dict(os.environ, {}, clear=True),
        ):
            with self.assertRaises(RuntimeError) as error_context:
                setup_test_authentication("odoo-test")
