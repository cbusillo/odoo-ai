...
mcp__codex__codex(
    ...
    - Odoo patterns: @docs/agent-patterns/odoo-engineer-patterns.md
...
mcp__codex__codex_reply(prompt="Now optimize it", sessionId=session_id)
```
...
mcp__codex__codex(
    prompt="Your task",
    model=env("OPENAI_PRIMARY_MODEL")  # Only when you must override the default
)

# Note: prefer the CLI default; override via env only when necessary.
```
...
1. **Use a large‑context profile/model** only when the task truly needs it
