...
class QualityControlOrder(models.Model):
    ...
    stage_id = fields.Many2one(
        "service.quality.control.order.stage",
    ...
    def _compute_stage_id(self) -> None:
        stage_model = self.env["service.quality.control.order.stage"]
...
class QualityControlOrderStage(models.Model):
    _name = "service.quality.control.order.stage"
    _description = "Quality Control Order Stage"
    _order = "sequence, id"

    name = fields.Char(required=True)
    code = fields.Selection(QUALITY_CONTROL_ORDER_STATES, required=True, index=True)
    ...
    _code_unique = models.Constraint(
        "unique(code)",
        "Quality control order stage code must be unique.",
    )
