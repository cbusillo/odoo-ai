...
class TestProductExporter(IntegrationTestCase):
    ...
    def test_publish_product_not_skipped_when_test_store_disabled_text(self) -> None:
    ...
    def test_sync_images_after_export_assigns_media_ids_without_name_error(self) -> None:
        from ..fixtures.factories import ProductFactory

        product = ProductFactory.create(self.env).product_variant_id
        image = self.env["product.image"].create(
            {
                "name": "Original alt",
                "image_1920": self._get_valid_image_base64(),
        ...
        self.exporter._sync_images_after_export(product, shopify_product)

        self.assertEqual(image.get_external_system_id("shopify", "media"), "111")
        self.assertEqual(image.name, "Updated alt")
