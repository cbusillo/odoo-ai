...
class _RecordIds(Protocol):
    ids: list[int]


class _ExternalIdLookup(Protocol):
    def get_external_id(self) -> dict[int, str]: ...


class _BrowsableModel(Protocol):
    def browse(self, record_ids: list[int]) -> _RecordIds: ...
...
class TestMigrationStatePreservation(UnitTestCase):
    ...
    @staticmethod
    def _xmlid_names(records: object) -> list[str]:
        external_id_map = cast(_ExternalIdLookup, records).get_external_id()
    ...
    def _recordset_from_xmlids(self, model_name: str, xmlids: list[str]) -> _RecordIds:
        model: _BrowsableModel = self.env[model_name]
        record_ids: list[int] = []
        for xmlid in xmlids:
    ...
    def test_snapshot_and_restore_verified_marine_motors_split_state(self) -> None:
        ...
        trim_tilt_status.write(
            {"selection_options": [(6, 0, self._recordset_from_xmlids("motor.test.selection", trim_tilt_selection_xmlids).ids)]}
        )
        trim_unit.write(
            {
                "hidden_tests": [
        ...
        lower_unit.write(
            {"hidden_tests": [(6, 0, self._recordset_from_xmlids("motor.test.template", lower_unit_hidden_test_xmlids).ids)]}
        )
        ...
        trim_tilt_status.write(
            {
                "selection_options": [
                    (
                        6,
                        0,
                        self._recordset_from_xmlids("motor.test.selection", trim_tilt_selection_xmlids + ["option_bad_motor"]).ids,
                    )
                ]
            }
        )
