...
ExternalIdRecordModel = TypeVar("ExternalIdRecordModel", bound="odoo.model.external_id_mixin")
...
DISCORD_SYSTEM_CODE = "discord"
REPAIRSHOPR_SYSTEM_CODE = "repairshopr"
TIMECLOCK_SYSTEM_CODE = "timeclock"
...
class CmDataImporter(models.Model):
    ...
    def _import_accounts(
        ...
        partner_model: "odoo.model.res_partner" = self.env["res.partner"].sudo().with_context(IMPORT_CONTEXT)
        price_list_model: "odoo.model.integration_cm_data_price_list" = self.env[
            "integration.cm_data.price.list"
        ].sudo().with_context(IMPORT_CONTEXT)
    ...
    def _import_employees(
        ...
        for row in employee_rows:
            ...
            if employee_record:
                self._sync_employee_external_ids(employee_record, row)
    ...
    @staticmethod
    def _sync_employee_external_ids(
        employee_record: "odoo.model.hr_employee",
        row: CmDataEmployee,
    ) -> None:
        external_id_model = employee_record.env["external.id"].sudo().with_context(active_test=False)
        external_system_model = employee_record.env["external.system"]
        external_id_values = (
            (TIMECLOCK_SYSTEM_CODE, row.timeclock_id),
            (REPAIRSHOPR_SYSTEM_CODE, row.repairshopr_id),
            (DISCORD_SYSTEM_CODE, row.discord_id),
        )
        for system_code, external_id_value in external_id_values:
            normalized_external_id = str(external_id_value) if external_id_value is not None else ""
            if normalized_external_id:
                system_defaults = EMPLOYEE_EXTERNAL_SYSTEM_DEFAULTS[system_code]
                system = external_system_model.ensure_system(
                    code=system_code,
                    name=system_defaults["name"],
                    id_format=system_defaults["id_format"],
                    sequence=system_defaults["sequence"],
                    url=system_defaults["url"],
                    active=True,
                ...
                if (
                    ...
                    and conflicting_external_id.res_model == employee_record._name
                    and conflicting_external_id.res_id != employee_record.id
                    and conflicting_external_id.active
                ):
                    raise RuntimeError(
                        f"Unable to sync {system_code} external ID '{normalized_external_id}' for hr.employee({employee_record.id}): "
    ...
    def _get_or_create_by_external_id_with_sync(
        ...
        *,
        ...
        resolved_resource = resource or record_model.get_bound_external_resource_name(binding_name)
        if updated_at and not self._should_process_external_row(system, external_id_value, resolved_resource, updated_at):
            # noinspection PyUnresolvedReferences
            # False positive: record_model is constrained to external.id.mixin callers at runtime.
            existing_record = (
                record_model.search_by_bound_external_id(external_id_value, binding_name)
                if resource is None
                else record_model.search_by_external_id(EXTERNAL_SYSTEM_CODE, external_id_value, resolved_resource)
            )
            if existing_record:
        ...
        # False positive: record_model is constrained to external.id.mixin callers at runtime.
        record = (
            record_model.get_or_create_by_bound_external_id(external_id_value, values, binding_name)
            if resource is None
            else record_model.get_or_create_by_external_id(
                EXTERNAL_SYSTEM_CODE,
                external_id_value,
                values,
        ...
        self._mark_external_id_synced(system, external_id_value, resolved_resource, sync_timestamp)
        return record
