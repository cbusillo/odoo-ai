...
class TestOrderShippingImport(IntegrationTestCase):
    ...
    def test_import_order_with_shipping_variation(self, mock_import_customer: common.MagicMock) -> None:
        ...
        for index, (shipping_title, expected_carrier_name) in enumerate(variations_to_test, start=1):
            shopify_order_id = str(880000 + index)
            order_data = create_shopify_order_response(
            ...
            order = self.env["sale.order"].search_by_bound_external_id(shopify_order_id)
