...
from httpx import Client, Timeout, Limits, Request, Response, RequestError
...
class ShopifyService:
    ...
    def _create_http_client(self, api_token: str) -> Client:
        ...
        def _is_idempotent_request(request: Request) -> bool:
            ...
            if not normalized:
                return False
            if normalized.startswith("mutation") or normalized.startswith("subscription"):
                return False
            if normalized.startswith("query") or normalized.startswith("{"):
                return True
            return False
        ...
        def send_with_retry(request: Request, **kwargs: object) -> Response:
            ...
            for attempt in range(self.MAX_RETRY_ATTEMPTS + 1):
                try:
                    ...
                    if not _is_idempotent_request(request):
                        raise ShopifyApiError(f"Non-idempotent request failed for {request.url}") from error
                    if attempt == self.MAX_RETRY_ATTEMPTS:
                        raise ShopifyApiError(f"Max retries reached for {request.url}") from error
                    wait_time = min(self.MAX_SLEEP_TIME, self.MIN_SLEEP_TIME * 2**attempt)
                    _logger.warning(
                        "Retry %s for sync %s %s (%s); sleeping %.2fs",
                        attempt + 1,
                        self.sync_record.id,
                        request.url,
